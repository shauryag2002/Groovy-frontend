  ...
  useEffect(() => {
    ...
    const postfunc = async () => {
      const postdata = await axios.get(
        `https://groovybackend-shauryag2002.vercel.app/post/${params.id}`
      );
      // .then((data) => {
      setPost(postdata.data);
      // });
      const userOne = await axios.get(
        "https://groovybackend-shauryag2002.vercel.app/user?userId=" +
      ...
      <div className="container">
        <div className="center">
          <div className="contain">
            <Link to={"/profile/" + user.username} className="containery">
              <img
                src={
                  user.profilePicture
                    ? user.profilePicture
          ...
          <br />
          <button id="comments" onClick={() => commentViewer(post._id)}>
            {post.comments ? post.comments.length : 0} comments
          </button>
          <div className="commenter">
            <div id={`commentView${post._id}`} style={{ display: "none" }}>
              <input
                type="text"
                className="comment"
              ...
              />
              <button onClick={() => commentHandler(post._id)}>Comment</button>

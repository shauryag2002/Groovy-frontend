...
const Popup_following = (props) => {
  const [userall, setUserall] = useState([]);
  const params = useParams();
  const [follow, setFollow] = useState("");
  const [fllw, setFllw] = useState("UNFOLLOW");
  ...
  useEffect(() => {
    const userInfo = async () => {
      ...
      setUserall(user.data.followings);
      // user.data.followers.forEach(async (usr, i) => {
      //   const followBtn = document.getElementById("followBtn");
      //   const followee = await axios.post(
      //     `https://groovybackend-shauryag2002.vercel.app/user/${usr.userId}/setfollow`,
      //     {
      //       userId: localStorage.getItem("userId"),
      //     }
      //   );
      for (let i = 0; i < user.data.followings.length; i++) {
        const followBtn = document.getElementById("followBtn" + i);
        const element = user.data.followings[i];
...
export default Popup_following;
